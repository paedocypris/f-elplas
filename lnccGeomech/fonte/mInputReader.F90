        ...
        if (iprtin.eq.0) then
            if (lzero) then
                ...
                if (j.eq.1) then
                    ...
                    call printd (f,ndof,numnp,iecho)
                end if
            endif
        endif
100 continue
...
2000 format(6x,i10,5x,6(5x,i10))
    ...
    !! @param iprtin     iprtin
    subroutine leituraGeracaoCoordenadasDS(x, nsd, numnp, iprtin, iecho)

    implicit none

    integer*4, intent(in)   :: nsd, numnp, iprtin, iecho
...
2000 format(6x,i10,10x,3(1pe15.8,2x))
    ...
    integer*4 number_of_includes, line_index, shift_lines
    ...
    do while ( line_index <= number_of_lines)
        if (current_include_index <= number_of_includes) then
    ...
    !! @param default_value Valor default.
    subroutine readStringKeywordValue(keyword, value, value_default, ierr)
    implicit none
    character(50) keyword
    ...
    !! @param var_n         Valor n.
    subroutine readOutFlagKeyword(keyword, target, var_out, var_n, reserv, ierr)
    implicit none
    character(50) keyword
    ...
    keyword_line = keyword_line + 1
    read(file_lines(keyword_line), "(a)") reserv
...
1000 format(16i10,10x,14i10)
    ...
    subroutine genelFacesDS(keyword_name, conecElem, nen, nelx, nely, ierr)
    use mGlobaisEscalares
    use mMalha, only: numel

    implicit none
    integer*4:: nen, nelx, nely
...
1000 format(16i10,10x,14i10)
    ...
    keyword_name = "npint"
    call readIntegerKeywordValue(keyword_name, npint, npint, ierr)
    end subroutine !***************************************************************************************************
