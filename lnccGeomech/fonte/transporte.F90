...
!     Scheme: KT
      ...
      use mMalha,            only: x, nsd, numnp, numnpReserv, numel, numelReserv, nen, numLadosElem, numLadosReserv
...
!     
         ...
         if (n(1)==nel) then
             if(vel(1).lt.eps) then
                fluxo(1,1)=f(unb(1),vel(1),xkb(1), P(1))
                fluxo(2,1)=f(uel(1),vel(1),xk    , PC)
             else
                fluxo(1,1)=f(uf(nel),vel(1),xk   , PC)
                fluxo(2,1)=f(uf(nel),vel(1),xk   , P(1))
             end if
          else
             fluxo(1,1)=f(unb(1),vel(1),xkb(1)   , P(1))
             fluxo(2,1)=f(uel(1),vel(1),xk       , PC)
...
!
            if (n(2)==nel) then
               if(vel(2).lt.eps) then
                  fluxo(1,2)=g(unb(2),vel(2),xkb(2), P(2))
                  fluxo(2,2)=g(uel(2),vel(2),yk    , PC)
               else
                  fluxo(1,2)=g(uf(nel),vel(2),yk   , PC)
                  fluxo(2,2)=g(uf(nel),vel(2),yk   , PC)
               end if
            else
               fluxo(1,2)=g(unb(2),vel(2),xkb(2), P(2))
               fluxo(2,2)=g(uel(2),vel(2),yk    , PC) 
...
!
            if (n(3)==nel) then
               if(vel(3).gt.eps) then
                  fluxo(1,3)=f(unb(3),vel(3),xkb(3), P(3))
                  fluxo(2,3)=f(uel(3),vel(3),xk    , PC)
               else
                  fluxo(1,3)=f(uf(nel),vel(3),xk   , PC)
                  fluxo(2,3)=f(uf(nel),vel(3),xk   , PC)
               end if
            else
               fluxo(1,3)=f(uel(3),vel(3),xk    , PC)
               fluxo(2,3)=f(unb(3),vel(3),xkb(3), P(3))
            end if

            if (n(4)==nel) then
               if(vel(4).gt.eps) then
                  fluxo(1,4)=g(unb(4),vel(4),xkb(4) , P(4))
                  fluxo(2,4)=g(uel(4),vel(4),yk     , PC)
               else
                  fluxo(1,4)=g(uf(nel),vel(4),yk    , PC)
                  fluxo(2,4)=g(uf(nel),vel(4),yk    , PC)
               end if
            else
               fluxo(1,4)=g(uel(4),vel(4),yk        , PC)
               fluxo(2,4)=g(unb(4),vel(4),xkb(4)    , P(4))
            endif
!     
          if(nsd==3) then
            if (n(5)==nel) then
               if(vel(5).lt.eps) then
                  fluxo(1,5)=h(unb(5),vel(5),xkb(5), P(5))
                  fluxo(2,5)=h(uel(5),vel(5),zk    , PC)
               else
                  fluxo(1,5)=h(uf(nel),vel(5),zk   , PC)
                  fluxo(2,5)=h(uf(nel),vel(5),zk   , PC)
               end if
            else
               fluxo(1,5)=h(unb(5),vel(5),xkb(5)   , P(5))
               fluxo(2,5)=h(uel(5),vel(5),zk       , PC)
...
!     
            if (n(6)==nel) then
               if(vel(6).gt.eps) then
                  fluxo(1,6)=h(unb(6),vel(6),xkb(6), P(6))
                  fluxo(2,6)=h(uel(6),vel(6),zk    , PC)
               else
                  fluxo(1,6)=h(uf(nel),vel(6),zk   , PC)
                  fluxo(2,6)=h(uf(nel),vel(6),zk   , PC)
               end if
            else
               fluxo(1,6)=h(uel(6),vel(6),zk       , PC)
               fluxo(2,6)=h(unb(6),vel(6),xkb(6)   , P(6))
            end if
          endif

          satElemAnt(nel)=udd_rt(dt,ns,hx,hy,hz,uc,uel,unb,vel,fluxo,p,pc,xk,yk,zk,xkb,nel)
!
      end do
!
      contains
...
!
       ...
       use mMalha, only: xc, numLadosElem,conecNodaisElem, numLadosReserv, numelReserv
!
      implicit none
!     
...
! 
         ...
         if (n(1)==nel) then
             if(vel(1).lt.eps) then
                ...
                fluxo(1,1)=f(uf(nel),vel(1),xk   , PC)
                fluxo(2,1)=f(uf(nel),vel(1),xk   , PC)
             end if
          else
             fluxo(1,1)=f(unb(1),vel(1),xkb(1)   , P(1))
             fluxo(2,1)=f(uel(1),vel(1),xk       , PC)
